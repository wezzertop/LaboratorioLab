...
let lines = fs.readFileSync('app/dashboard/tests/asfalto/page.tsx', 'utf8').split('\n');
...
if (printStartIdx > -1 && printEndIdx > -1) {
    ...
    // Start searching for return (`  return (`) AFTER the isSavingIdx to make sure we're inside AsphaltTestContent
    ...
    const webViewIdx = lines.findIndex(l => l.includes('{/* VISTA WEB MAIN */}'));
    const modalLines = [
        '      {/* MODAL VISTA PREVIA */}',
        '      {showPreview && (',
    ...
    const saveBtnIdx = lines.findIndex((l, i) => i > isSavingIdx && l.includes('Guardar Ensaye'));
    ...
    fs.writeFileSync('app/dashboard/tests/asfalto/page.tsx', lines.join('\n'));
    console.log('Fixed asfalto!');
} else {
    console.log('Indices not found in asfalto', printStartIdx, printEndIdx);
}
